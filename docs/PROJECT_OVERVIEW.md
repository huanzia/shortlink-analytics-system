...
- 删除短链后的布隆过滤器重建 / 移除策略
- `tb_link_uv_record` 正式落库去重
- 更复杂的消息幂等
- 死信队列 / 重试队列
- 浏览器 / OS / referer 的更精细解析
- 更复杂的多维交叉分析
- 自定义时间范围趋势查询
- 实时排行榜（例如 Redis ZSet）
- 后台页面
...
- 完整实现了“短链创建 -> 跳转 -> 异步埋点 -> PV/UV 统计 -> 统计查询”的闭环链路
- 在跳转主链路中组合使用 Redis 缓存、空值缓存和布隆过滤器，增强缓存防穿透能力
- 使用 RabbitMQ 将访问埋点与跳转主流程解耦，降低同步请求链路压力
- 基于 Redis Set 实现基础版 UV 去重，并落到数据库 `uv_count`
- 提供热门排行、趋势统计和维度聚合接口，具备较完整的演示和复盘价值
- 项目采用分层结构和统一异常/结果封装，便于扩展和面试讲解
